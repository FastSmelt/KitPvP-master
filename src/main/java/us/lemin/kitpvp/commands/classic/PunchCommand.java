...
public class PunchCommand extends ClassicCommand {
    private final KitPvPPlugin plugin;

    public PunchCommand(KitPvPPlugin plugin) {
        super("punch", plugin);
        this.plugin = plugin;
    }
    ...
    public void execute(Player player, String[] args) {
        ...
        int punchLevel = player.getItemInHand().getEnchantments().get(Enchantment.ARROW_KNOCKBACK) == null ?
                0 : player.getItemInHand().getEnchantments().get(Enchantment.ARROW_KNOCKBACK);
        ...
        int cost = Buffs.PUNCH.getPriceByLevel(punchLevel);
        ...
        player.getItemInHand().addEnchantment(Enchantment.ARROW_DAMAGE, punchLevel + 1);
        player.sendMessage(CC.PRIMARY + "Your sword has been enchanted with Punch " + CC.ACCENT + ++punchLevel + CC.PRIMARY + " for " + CC.ACCENT + cost + " credits.");
